                                  ...
                                  const half* A, const half* B, half* C)
{
	...
	fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> frag_a[TILES_PER_WARP];
	fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, row_major> frag_b[TILES_PER_WARP];
	fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, float> acc[TILES_PER_WARP];
	...
	for (int k_tile = 0; k_tile < K; k_tile += BLOCK_SIZE_K) {
		...
		for (int k_step = 0; k_step < BLOCK_SIZE_K; k_step += WMMA_K) {
			for (int t = 0; t < TILES_PER_WARP; t++) {
				...
				int smem_a_row = tile_m * WMMA_M;
				int smem_a_col = k_step;
				
				int smem_b_row = k_step;
				int smem_b_col = tile_n * WMMA_N;
				
				int global_row = block_m * BLOCK_SIZE_M + tile_m * WMMA_M;
	...
	// Epilogue: C = alpha * (A*B) + beta * C - handle multiple tiles per warp
	for (int t = 0; t < TILES_PER_WARP; t++) {
		int tile_m = warp_m * TILES_PER_WARP + t;
		int tile_n = warp_n;
		
		int row_start = block_m * BLOCK_SIZE_M + tile_m * WMMA_M;
		int col_start = block_n * BLOCK_SIZE_N + tile_n * WMMA_N;
		...
		if (row_start < M && col_start < N) {
			...
			#pragma unroll
			for (int idx = 0; idx < acc[t].num_elements; idx++) {
				acc[t].x[idx] = fmaf(acc[t].x[idx], alpha_f, frag_c.x[idx] * beta_f);
			}
			...
			#pragma unroll
			for (int idx = 0; idx < acc[t].num_elements; idx++) {
				acc_half.x[idx] = __float2half(acc[t].x[idx]);
			}
                     ...
                     half* A, half* B, half* C, half* C_outputFromGpu)
{
	...
	// Use synchronous cudaMemcpy for baseline
	...
	// Layout: 8 warps in M direction, 2 warps in N direction (8x2 = 16 warps)
