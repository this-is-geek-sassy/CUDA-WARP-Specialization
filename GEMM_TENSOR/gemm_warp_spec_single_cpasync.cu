...
#define PERCENT_DIFF_ERROR_THRESHOLD 0.1
                                  ...
                                  const half* A, const half* B, half* C)
{
	...
	if (is_load_warp) {
		// 4 DMA WARPS: Use cp.async for asynchronous loading from global to shared
		int num_load_threads = NUM_LOAD_WARPS * warpSize;  // 128 threads
		int load_thread_id = warp_id * warpSize + lane_id;
		
		for (int k_tile = 0; k_tile < K; k_tile += BLOCK_SIZE_K) {
			// Load A tile using cp.async (loads 4 bytes = 2 halfs at a time for alignment)
			...
			for (int i = load_thread_id * 2; i < tile_size_A; i += num_load_threads * 2) {
				...
				if (global_row < M && global_col < K && (col + 1) < BLOCK_SIZE_K) {
					uint32_t smem_addr = __cvta_generic_to_shared(&shared_A[row][col]);
					...
					if (col + 1 < BLOCK_SIZE_K) {
						if (global_row < M && global_col + 1 < K) {
							shared_A[row][col + 1] = A[global_row * K + global_col + 1];
						} else {
							shared_A[row][col + 1] = __float2half(0.0f);
						}
					}
				}
			}
			...
			for (int i = load_thread_id * 2; i < tile_size_B; i += num_load_threads * 2) {
				...
				if (global_row < K && global_col < N && (col + 1) < BLOCK_SIZE_N) {
					uint32_t smem_addr = __cvta_generic_to_shared(&shared_B[row][col]);
					...
					if (col + 1 < BLOCK_SIZE_N) {
						if (global_row < K && global_col + 1 < N) {
							shared_B[row][col + 1] = B[global_row * N + global_col + 1];
						} else {
							shared_B[row][col + 1] = __float2half(0.0f);
						}
					}
				}
			}
			
			// Commit all pending cp.async operations and wait for completion
			asm volatile("cp.async.commit_group;\n");
			asm volatile("cp.async.wait_group 0;\n");  // Wait for all groups to complete
			
			__syncthreads();  // Load done, compute warps can proceed
			__syncthreads();  // Wait for compute to finish before loading next tile
		}
		
	} else {
		// COMPUTE WARPS: 12 warps for computation (warp 4-15)
		int compute_warp_local_id = warp_id - NUM_LOAD_WARPS;
		int total_positions = BLOCK_TILES_M * BLOCK_TILES_N;  // 8×4 = 32 tiles for 128x64 block
		
		// Distribute 32 positions among 12 compute warps
		...
		fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, float> acc[3];  // Max 3 tiles per warp (8 warps get 3, 4 warps get 2)
		...
		for (int k_tile = 0; k_tile < K; k_tile += BLOCK_SIZE_K) {
			__syncthreads();  // Wait for load warps
			
			// Compute for all positions assigned to this warp
			for (int p = 0; p < positions_per_warp; p++) {
				int pos = start_pos + p;
				int warp_m = pos / BLOCK_TILES_N;  // 8 tiles in M direction
				int warp_n = pos % BLOCK_TILES_N;  // 4 tiles in N direction
                     ...
                     half* A, half* B, half* C, half* C_outputFromGpu)
{
	...
	// Layout: 8 warps in M direction, 2 warps in N direction (but warp-spec uses different mapping)
