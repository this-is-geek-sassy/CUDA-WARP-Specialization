...
__global__ void convolution3D_kernel(int ni, int nj, int nk, DATA_TYPE* A, DATA_TYPE* B, int i)
{
	...
	// Padding the innermost dimension: (32 + 2) + 1 = 35 instead of 34
	// This prevents threads in the same warp from accessing the same bank
	__shared__ DATA_TYPE tile[3][DIM_THREAD_BLOCK_Y + 2][DIM_THREAD_BLOCK_X + 3];

	// Constant coefficients (use const for better register usage)
	const DATA_TYPE c11 = +2, c21 = +5, c31 = -8;
	const DATA_TYPE c12 = -3, c22 = +6, c32 = -9;
	const DATA_TYPE c13 = +4, c23 = +7, c33 = +10;

	// Load tile for 3 i-slices into shared memory
	// Load i-1 slice
	for (int dj = ty; dj < DIM_THREAD_BLOCK_Y + 2; dj += blockDim.y) {
		for (int dk = tx; dk < DIM_THREAD_BLOCK_X + 2; dk += blockDim.x) {
			...
			if (i > 0 && global_j >= 0 && global_j < nj && global_k >= 0 && global_k < nk) {
				tile[0][dj][dk] = A[(i - 1)*(NK * NJ) + global_j*NK + global_k];
			} else {
				tile[0][dj][dk] = 0.0f;
			}
		}
	...
	for (int dj = ty; dj < DIM_THREAD_BLOCK_Y + 2; dj += blockDim.y) {
		for (int dk = tx; dk < DIM_THREAD_BLOCK_X + 2; dk += blockDim.x) {
			...
			if (i >= 0 && i < ni && global_j >= 0 && global_j < nj && global_k >= 0 && global_k < nk) {
				tile[1][dj][dk] = A[i*(NK * NJ) + global_j*NK + global_k];
			} else {
				tile[1][dj][dk] = 0.0f;
			}
		}
	...
	for (int dj = ty; dj < DIM_THREAD_BLOCK_Y + 2; dj += blockDim.y) {
		for (int dk = tx; dk < DIM_THREAD_BLOCK_X + 2; dk += blockDim.x) {
			...
			if (i < ni - 1 && global_j >= 0 && global_j < nj && global_k >= 0 && global_k < nk) {
				tile[2][dj][dk] = A[(i + 1)*(NK * NJ) + global_j*NK + global_k];
			} else {
				tile[2][dj][dk] = 0.0f;
			}
		}
	...
	if ((i < (_PB_NI-1)) && (j < (_PB_NJ-1)) && (k < (_PB_NK-1)) && (i > 0) && (j > 0) && (k > 0))
	{
		...
		DATA_TYPE result = 
			...
			c11 * tile[0][sj-1][sk-1] + c13 * tile[2][sj-1][sk-1] +
			c21 * tile[0][sj-1][sk-1] + c23 * tile[2][sj-1][sk-1] +
			c31 * tile[0][sj-1][sk-1] + c33 * tile[2][sj-1][sk-1] +
			c12 * tile[1][sj-1][sk] +
			c22 * tile[1][sj][sk] +
			c32 * tile[1][sj+1][sk] +
			c11 * tile[0][sj-1][sk+1] +
			c13 * tile[2][sj-1][sk+1] +
			c21 * tile[0][sj][sk+1] +
			c23 * tile[2][sj][sk+1] +
			c31 * tile[0][sj+1][sk+1] +
			c33 * tile[2][sj+1][sk+1];
