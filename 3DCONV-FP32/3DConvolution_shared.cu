...
__global__ void convolution3D_kernel(int ni, int nj, int nk, DATA_TYPE* A, DATA_TYPE* B, int i)
{
	...
	// Shared memory for 3 consecutive i-slices (i-1, i, i+1)
	// Each slice needs (blockDim.y + 2) × (blockDim.x + 2) for halo
	__shared__ DATA_TYPE tile[3][DIM_THREAD_BLOCK_Y + 2][DIM_THREAD_BLOCK_X + 2];
