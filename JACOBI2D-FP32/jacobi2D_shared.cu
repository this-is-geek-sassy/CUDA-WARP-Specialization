...
{
	// Shared memory tile with halo cells (34x18 for 32x16 block)
	...
	// Load halo cells (top, bottom, left, right)
	if (ty == 0 && i > 0) // Top halo
	{
		tile[0][tx + 1] = A[(i - 1) * N + j];
	}
	if (ty == blockDim.y - 1 && i < n - 1) // Bottom halo
	{
		tile[ty + 2][tx + 1] = A[(i + 1) * N + j];
	}
	if (tx == 0 && j > 0) // Left halo
	{
		tile[ty + 1][0] = A[i * N + (j - 1)];
	}
	if (tx == blockDim.x - 1 && j < n - 1) // Right halo
	{
