                                  ...
                                  const half* A, const half* B, half* C)
{
	...
	// DOUBLE BUFFERING with bank conflict optimization
	...
	// Tile across K dimension with DOUBLE BUFFERING
	...
	for (int tile_idx = 0; tile_idx < num_k_tiles; tile_idx++) {
		...
		int buffer_idx = tile_idx % 2;  // PING-PONG between buffer 0 and 1
		
		// Cooperative loading of A tile into shared memory (DOUBLE BUFFERED)
		...
		// Cooperative loading of B tile into shared memory (DOUBLE BUFFERED)
		...
		// Compute using shared memory (USE CURRENT BUFFER)
