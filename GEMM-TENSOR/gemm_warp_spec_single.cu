                                  ...
                                  const half* A, const half* B, half* C)
{
	...
	if (is_load_warp) {
		// LOAD WARPS: Only load data into shared memory
		...
		// COMPUTE WARPS: Only perform computation
		int compute_warp_local_id = warp_id - NUM_LOAD_WARPS;
		int total_positions = 16;  // 4×4 grid of WMMA tiles
		
		// Distribute positions among compute warps
		...
		fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, float> acc[2];  // Max 2 tiles per warp (for 12 warps)
