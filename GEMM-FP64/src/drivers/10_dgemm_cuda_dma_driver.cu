...
bool dgemm_cuda_dma_driver(float alpha, float beta, int M, int N, int K, float *hA, float *hB, float *hC, bool debug)
{
  const size_t max_optin_limit = get_max_optin_limit<0>();

  const unsigned int BM = BM10;
  const unsigned int BK = BK10;
  const unsigned int BN = BN10;
  const unsigned int TM = TM10;
  const unsigned int TN = TN10;
  const unsigned int TK = TK10;
  const unsigned int WARP_SIZE = WARP_SIZE10;
  const unsigned int NUM_LOAD_WARPS = NUM_LOAD_WARPS10;
  const unsigned int BDM = BM / TM;
  const unsigned int BDN = BN / TN;
  const unsigned int NUM_COMPUTE_WARPS = (BDM * BDN) / WARP_SIZE;
  const unsigned int NUM_COMPUTE_THREADS = NUM_COMPUTE_WARPS * WARP_SIZE;
  const unsigned int NUM_LOAD_THREADS = WARP_SIZE * NUM_LOAD_WARPS;

  auto kernel = dgemm_cuda_dma<BM, BK, BN, TM, TN, TK, NUM_LOAD_WARPS, NUM_COMPUTE_WARPS, WARP_SIZE>;
  cudaFuncAttributes attr;
  cudaFuncGetAttributes(&attr, kernel);
  if (MAX_SMEM)
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, max_optin_limit);
  ...
  if (!CUDA_CHECK(cudaMalloc(&dA, M * K * sizeof(float))))
    goto cleanup;
  if (!CUDA_CHECK(cudaMalloc(&dB, K * N * sizeof(float))))
    goto cleanup;
  if (!CUDA_CHECK(cudaMalloc(&dC, M * N * sizeof(float))))
    goto cleanup;

  if (!CUDA_CHECK(cudaMemcpy(dA, hA, M * K * sizeof(float), cudaMemcpyHostToDevice)))
    goto cleanup;
  if (!CUDA_CHECK(cudaMemcpy(dB, hB, K * N * sizeof(float), cudaMemcpyHostToDevice)))
    goto cleanup;
  if (!CUDA_CHECK(cudaMemcpy(dC, hC, M * N * sizeof(float), cudaMemcpyHostToDevice)))
    goto cleanup;

  cudaEvent_t start, stop;
  float milliseconds;
  if (!CUDA_CHECK(cudaEventCreate(&start)))
    goto cleanup;
  if (!CUDA_CHECK(cudaEventCreate(&stop)))
    goto cleanup;

  if (!CUDA_CHECK(cudaEventRecord(start)))
    goto cleanup;
  kernel<<<gridDim, blockDim, sharedMemSize>>>(alpha, beta, M, N, K, dA, dB, dC);
  if (!CUDA_CHECK(cudaEventRecord(stop)))
    goto cleanup;
  ...
  if (!CUDA_CHECK(cudaEventElapsedTime(&milliseconds, start, stop)))
    goto cleanup;
  std::cout << milliseconds * 1000 << std::endl;

  if (!CUDA_CHECK(cudaMemcpy(hC, dC, M * N * sizeof(float), cudaMemcpyDeviceToHost)))
    goto cleanup;
