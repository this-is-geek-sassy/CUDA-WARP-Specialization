...
#include "kernels/2_dgemm_2d_tiled.cuh"
...
bool dgemm_2d_tiled_driver(float alpha, float beta, int M, int N, int K, float* hA, float* hB, float* hC, bool debug) {
  const size_t max_optin_limit = get_max_optin_limit<0>();
  
  const unsigned int BM = BM2;
  const unsigned int BK = BK2;
  const unsigned int BN = BN2;
  const unsigned int TM = TM2;
  const unsigned int TN = TN2;

  auto kernel = dgemm_2d_tiled<BM, BK, BN, TM, TN>;
